...
std::ostream &operator<<(std::ostream &stream, Dados &d){
    stream << "El valor del dado 1 es: "
    << d.getDado1()
    <<"\n"
    <<"El valor del dado 2 es: "
    <<d.getDado2()
    <<"\n";
    return stream;
}
std::istream &operator>>(std::istream &stream, Dados &d){
    d.d1_=d.d2_=-1;
    while(!d.setDado1(d.d1_)){
        std::cout << "Introduce un valor entre 1 y 6 para el dado 1 ";
        stream >> d.d1_;
    }
    while(!d.setDado2(d.d2_)){
        std::cout << "Introduce un valor entre 1 y 6 para el dado 2 ";
        stream >> d.d2_;
    }
    return stream;
}
